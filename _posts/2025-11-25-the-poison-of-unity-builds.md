...
- What does the compiler do when you build an input file into an executable?
    - `gcc program.c -o program.out`
    - preprocesses `program.c` into a temporary source file
    - compiles the preprocessed `program.c` into a temporary object file
    - links the object file with its dependencies
    - assembles the linked object files into a binary

- What does it do when you build multiple input files into an executable?
    - `gcc a.c b.c -o program.out`
    - for each source file:
        - preprocesses the file into a temporary source file
        - compiles the preprocessed file into a temporary object file
    - links the object files together, and with their dependencies
    - assembles the linked object files into a binary

- We can make it faster (incremental/parallel builds)
    - less file write/reads
        - `gcc a.c b.c -pipe -o program.out` to use pipes instead of files
    - parallelize source compilation with gnu make
        - we get incremental builds for free with make
    - optimize LTO with gnu make
        - link to https://www.codestudy.net/blog/can-gcc-use-multiple-cores-when-linking/ to show some other optimizations that could be made

- The year is 2007 (the magic of unity builds)
    - consumer CPUs had at most 4 cores with very little cache
        - so... we can at least parallelize a little bit, right?

    - SSDs weren't commonplace yet. HDDs are slow, and IO can't be parallelized on them.
        - so reading files during the preprocess step is slow, since we can't parallelize reading header files
        - writing object files during the compilation step is slow, especially since it cant really be parallelized

    - its a wonder how we managed to do computing 20 years ago, lol
    - so, how do we speed up compiling small and large projects alike?
        - skip all of that HDD IO and do it all at once!!!
        - by compiling everything as a single translation unit, the computer just loads everything it can into RAM and compiles it as a single object
        - N.B. unity builds were never about organizing your project. They were always about improving build times. It was always expected that you continued to produce released builds with a typical project organization with standard multi-unit practices like headers, header guards, external linkage, forward declarations, et cetera.

    - consumer RAM was much smaller, way slower, and often only single-channel.
        - we can't always preemptively load everything into RAM, especially with large projects
        - paging from/to HDD is so painfully slow
        - as a result, unity builds weren't perfect, but almost always were a boon to build times

- The year is 2025 (the poison of unity builds)
    - consumer CPUs are crazy
        - virtually always have at least 6 cores, and frequently have more than double that.
        - Modern CPUs have much more optimal and efficient instruction pipelines.
        - quadruple the average CPU cache from 2007

    - consumer SSDs are crazy
        - a fraction of the read/write speeds
        - no seeking
        - easily parallelized due to their speed even with random reads and writes
    
    - IO is virtually never the bottleneck anymore
        - source files can be compiled independently very quickly, in parallel
        - compiled objects can be cached as a file and only recompiled when they change
        - linkers are more complex than ever, and are virtually always the bottleneck when compiling
    
    - poison
        - the unity build kills compilation times. Source file compilation can't be parallelized. We're wasting the potential of our hardware.

        - the unity build can make organizing your project more complicated.
            - nowadays I often see developers recommend unity builds as a way to reduce the strain on organizing your project, and as a way to avoid some confusion with link-time errors.
            - If you commit to a header-less unity build, then you become dependent on the order in which source files are included in the UB file. As a result, two source files cannot circularly depend on one another's declarations.
            - To deal with this, you need to forward declare functions, and to relocate global variables and types into other source file which gets included in the UB file first... Wait, isn't that just a header file? So, its completely redundant.
            - If you commit to unity builds in local development but still use standard header file practices, you are missing out on a much faster and simpler build process.

- The cure (just use build tools)
    - Just use the build tools at your disposal, like make, cmake, nmake, Visual Studio's MSBuild, et cetera. They all support parallelization.
